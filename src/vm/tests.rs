...
mod bytecode_tests {
    ...
    fn execute_bytecode_program(instructions: Vec<Instruction>, database: &Database) -> SqawkResult<Option<Table>> {
    ...
    fn test_basic_flow_opcodes() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::Init, 0, 1, 0, None, Some("Initialize VM".to_string())),
            
            // Load integer 42 into register 1
            create_instruction(OpCode::Integer, 42, 1, 0, None, Some("Load value 42".to_string())),
            
            // Return result row with register 1
            create_instruction(OpCode::ResultRow, 1, 1, 0, None, Some("Return result".to_string())),
            
            // Halt execution
            create_instruction(OpCode::Halt, 0, 0, 0, None, Some("Stop execution".to_string())),
        ];
        
        // Execute the program
        let result = execute_bytecode_program(instructions, &database).expect("Failed to execute program");
    ...
    fn test_string_opcode() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::String, 0, 1, 0, Some("test".to_string()), Some("Load string value".to_string())),
    ...
    fn test_null_opcode() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::Null, 0, 1, 0, None, Some("Load NULL value".to_string())),
    ...
    fn test_result_row_with_multiple_registers() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::String, 0, 2, 0, Some("test".to_string()), Some("Load string value".to_string())),
            
            // Load NULL into register 3
            create_instruction(OpCode::Null, 0, 3, 0, None, Some("Load NULL value".to_string())),
            
            // Return result row with registers 1, 2, 3
            create_instruction(OpCode::ResultRow, 1, 3, 0, None, Some("Return result".to_string())),
    ...
    fn test_table_operations() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::OpenRead, 1, 0, 0, Some("test_table".to_string()), Some("Open table".to_string())),
            
            // Rewind cursor 1, jump to end (instruction 7) if empty
            create_instruction(OpCode::Rewind, 1, 7, 0, None, Some("Move to first row".to_string())),
            
            // Loop start:
            // Read column 0 (id) into register 1
            create_instruction(OpCode::Column, 1, 0, 1, None, Some("Read ID column".to_string())),
            
            // Read column 1 (name) into register 2
            create_instruction(OpCode::Column, 1, 1, 2, None, Some("Read name column".to_string())),
            
            // Return result row with registers 1-2
            create_instruction(OpCode::ResultRow, 1, 2, 0, None, Some("Return result".to_string())),
            
            // Next row for cursor 1, jump to loop start (instruction 3) if more rows
            create_instruction(OpCode::Next, 1, 3, 0, None, Some("Move to next row".to_string())),
            
            // Close cursor 1
            create_instruction(OpCode::Close, 1, 0, 0, None, Some("Close cursor".to_string())),
        ...
        // Check first row
        assert_eq!(rows[0][0], Value::Integer(1), "First row, first column should be 1");
        assert_eq!(rows[0][1], Value::String("Alice".to_string()), "First row, second column should be 'Alice'");
        
        // Check second row
        assert_eq!(rows[1][0], Value::Integer(2), "Second row, first column should be 2");
        assert_eq!(rows[1][1], Value::String("Bob".to_string()), "Second row, second column should be 'Bob'");
    }
    ...
    fn test_goto_opcode() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::Integer, 10, 1, 0, None, Some("Load value 10".to_string())),
            
            // Jump unconditionally to instruction 4, skipping the next instruction
            create_instruction(OpCode::Goto, 0, 4, 0, None, Some("Jump to instruction 4".to_string())),
            
            // This instruction should be skipped: load integer 20 into register 1
            create_instruction(OpCode::Integer, 20, 1, 0, None, Some("Load value 20 (should be skipped)".to_string())),
    ...
    fn test_multiple_result_rows() {
        ...
        let instructions = vec![
            ...
            create_instruction(OpCode::Integer, 1, 1, 0, None, Some("Load value 1".to_string())),
            
            // Return first result row
            create_instruction(OpCode::ResultRow, 1, 1, 0, None, Some("Return first result".to_string())),
            
            // Load integer 2 into register 1
            create_instruction(OpCode::Integer, 2, 1, 0, None, Some("Load value 2".to_string())),
            
            // Return second result row
            create_instruction(OpCode::ResultRow, 1, 1, 0, None, Some("Return second result".to_string())),
            
            // Load integer 3 into register 1
            create_instruction(OpCode::Integer, 3, 1, 0, None, Some("Load value 3".to_string())),
            
            // Return third result row
            create_instruction(OpCode::ResultRow, 1, 1, 0, None, Some("Return third result".to_string())),
        ...
        // Check first row
        match &rows[0][0] {
            Value::Integer(val) => assert_eq!(*val, 1, "First row should have value 1, got {}", val),
            other => panic!("Expected Integer type, got {:?}", other),
        }
        
        // Check second row
        match &rows[1][0] {
            Value::Integer(val) => assert_eq!(*val, 2, "Second row should have value 2, got {}", val),
            other => panic!("Expected Integer type, got {:?}", other),
        }
        
        // Check third row
        match &rows[2][0] {
            Value::Integer(val) => assert_eq!(*val, 3, "Third row should have value 3, got {}", val),
            other => panic!("Expected Integer type, got {:?}", other),
        }
    }
    ...
    fn test_noop_opcode() {
        ...
        let instructions = vec![
            ...
            // No operation
            create_instruction(OpCode::Noop, 0, 0, 0, None, Some("No operation".to_string())),
