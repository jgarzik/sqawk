...
impl<'a> VmEngine<'a> {
    ...
    fn execute_instruction(&mut self, inst: &Instruction) -> SqawkResult<ExecuteResult> {
        match inst.opcode {
            ...
            OpCode::Eq => {
                // Equal comparison - jump if not equal
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
                
                // Check registers are in bounds
                if reg1 >= self.registers.len() || reg3 >= self.registers.len() {
                    return Err(SqawkError::VmError(format!("Register index out of bounds: {} or {}", reg1, reg3)));
                }
                
                // Compare values
                let equal = self.registers[reg1] == self.registers[reg3];
                if !equal {
                    // Values not equal, jump
                    Ok(ExecuteResult::Jump(jump_addr))
                } else {
                    // Values equal, continue
                    Ok(ExecuteResult::Continue)
                }
            },
            
            OpCode::Ne => {
                // Not equal comparison - jump if equal
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
                ...
                if equal {
                    ...
                    // Values not equal, continue
                    Ok(ExecuteResult::Continue)
                }
            },
            
            OpCode::Lt => {
                // Less than comparison - jump if greater than or equal
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
                ...
                let less_than = match (&self.registers[reg1], &self.registers[reg3]) {
                    ...
                    _ => return Err(SqawkError::VmError(format!("Unsupported comparison types: {:?} < {:?}", 
                        self.registers[reg1], self.registers[reg3]))),
                };
            ...
            OpCode::Le => {
                // Less than or equal comparison - jump if greater than
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
                ...
                // Compare values based on type
                let greater_than = match (&self.registers[reg1], &self.registers[reg3]) {
                    (Register::Integer(a), Register::Integer(b)) => a > b,
                    (Register::Float(a), Register::Float(b)) => a > b,
                    (Register::Integer(a), Register::Float(b)) => (*a as f64) > *b,
                    (Register::Float(a), Register::Integer(b)) => *a > (*b as f64),
                ...
                if greater_than {
                    ...
                    // Less than or equal, continue
                    Ok(ExecuteResult::Continue)
                }
            },
            
            OpCode::Gt => {
                // Greater than comparison - jump if less than or equal
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
            ...
            OpCode::Ge => {
                // Greater than or equal comparison - jump if less than
                let reg1 = inst.p1 as usize;
                let jump_addr = inst.p2 as usize;
                let reg3 = inst.p3 as usize;
                ...
                if less_than {
                    ...
                    // Greater than or equal, continue
                    Ok(ExecuteResult::Continue)
                }
            },
