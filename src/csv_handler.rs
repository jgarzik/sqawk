...
impl CsvHandler {
    ...
    pub fn save_table(&self, table_name: &str) -> SqawkResult<()> {
        ...
        let file_path = table.source_file()
            .ok_or_else(|| SqawkError::InvalidSqlQuery(
                format!("Table '{}' doesn't have a source file", table_name)
            ))?;
    ...
    fn parse_file_spec(&self, file_spec: &str) -> SqawkResult<(String, PathBuf)> {
        if let Some((table_name, file_path)) = file_spec.split_once('=') {
            ...
            path.file_name()
                .ok_or_else(|| SqawkError::InvalidFileSpec(
                    format!("Invalid file specification: {}", file_spec)
                ))?;
