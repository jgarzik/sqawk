        ...
        match *query.body {
            SetExpr::Select(ref select) => {
                ...
                if select.from.is_empty() {
                    if self.config.use_vm() {
                        // If using VM mode, delegate to VM implementation directly
                        // We'll use a simple string for the SQL since formatting isn't available in this version
                        let sql = "SELECT 1";  // Minimal implementation since we'll actually use the parsed statement
                        return crate::vm::execute_vm(sql, self.database, self.config.verbose());
                    } else {
                        // For non-VM implementation, create a synthetic single-row table with literals
                        ...
                        for (i, item) in select.projection.iter().enumerate() {
                            match item {
                                SelectItem::UnnamedExpr(expr) => {
                                    // For literals, make up a column name
                                    let col_name = format!("col{}", i + 1);
                                    column_names.push(col_name);
                                    
                                    // This would need more work to fully implement all expression types
                                    match expr {
                                        Expr::Value(value) => {
                                            match value {
                                                sqlparser::ast::Value::Number(val, _) => {
                                                    ...
                                                    if let Ok(int_val) = val.parse::<i64>() {
                                                        row_values.push(crate::table::Value::Integer(int_val));
                                                    } else if let Ok(float_val) = val.parse::<f64>() {
                                                        // Then try as float
                                                        row_values.push(crate::table::Value::Float(float_val));
                                                    } else {
                                                        // Fallback to string
                                                        row_values.push(crate::table::Value::Text(val.clone()));
                                                    }
                                                },
                                ...
                                SelectItem::ExprWithAlias { expr, alias } => {
                                    ...
                                    // Same as above for the expression evaluation
                                    match expr {
                                        Expr::Value(value) => {
                                            match value {
                                                sqlparser::ast::Value::Number(val, _) => {
