...
fn test_tab_delimiter() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("id,name,salary,department"), 
            "Header row should be present");
    assert!(stdout.contains("1,Alice,75000,Engineering"), 
            "Row for Alice should be present (salary > 70000)");
    assert!(stdout.contains("3,Charlie,85000,Engineering"), 
            "Row for Charlie should be present (salary > 70000)");
    
    // Check that we don't see rows with salary <= 70000
    assert!(!stdout.contains("Bob") || !stdout.contains("65000"), 
            "Should not contain Bob with salary 65000 (salary <= 70000)");
    assert!(!stdout.contains("David") || !stdout.contains("60000"), 
...
fn test_colon_delimiter() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check that header and desired rows are present
    assert!(stdout.contains("id,name,email"), 
            "Header row should be present");
    assert!(stdout.contains("10,Eve,eve@example.com"), 
            "Row for Eve should be present (id > 5)");
    
    // Check that we don't see rows with id <= 5
    assert!(!stdout.contains("Alice") || !stdout.contains("alice@example.com"), 
            "Should not contain Alice (id <= 5)");
    assert!(!stdout.contains("Bob") || !stdout.contains("bob@example.com"),  
            "Should not contain Bob (id <= 5)");
    assert!(!stdout.contains("Charlie") || !stdout.contains("charlie@example.com"), 
            "Should not contain Charlie (id <= 5)");
    assert!(!stdout.contains("David") || !stdout.contains("david@example.com"), 
