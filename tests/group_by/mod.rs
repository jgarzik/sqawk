...
fn test_basic_group_by() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("department,count,total_salary,avg_salary"));
    ...
    assert!(stdout.contains("Engineering,3,250000,83333.33"), "Engineering row incorrect");
    
    // Marketing should have 2 employees, total salary 135000, avg 67500
    assert!(stdout.contains("Marketing,2,135000,67500"), "Marketing row incorrect");
    
    // Sales should have 2 employees, total salary 125000, avg 62500
    assert!(stdout.contains("Sales,2,125000,62500"), "Sales row incorrect");
...
fn test_group_by_with_order_by() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("department,employee_count,avg_salary"));
    ...
    assert!(data_rows[0].contains("Engineering"), "First row should be Engineering with highest avg_salary");
    assert!(data_rows[1].contains("Marketing"), "Second row should be Marketing with middle avg_salary");
    assert!(data_rows[2].contains("Sales"), "Third row should be Sales with lowest avg_salary");
...
fn test_group_by_with_complex_expressions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("department,employee_count,total_salary,avg_salary,min_salary,max_salary"), 
            "Header row not found or incorrect");
    
    // Check each department is present and has correct employee count
    assert!(stdout.contains("Engineering,3,"), "Engineering row has incorrect employee count");
    assert!(stdout.contains("Marketing,2,"), "Marketing row has incorrect employee count");
    assert!(stdout.contains("Sales,2,"), "Sales row has incorrect employee count");
    
    // Check total salaries are correct
    assert!(stdout.contains(",250000,"), "Engineering total salary should be 250000");
    assert!(stdout.contains(",135000,"), "Marketing total salary should be 135000");
    assert!(stdout.contains(",125000,"), "Sales total salary should be 125000");
    
    // Check min/max values
    assert!(stdout.contains(",75000,90000"), "Engineering min/max salaries should be 75000/90000");
    assert!(stdout.contains(",65000,70000"), "Marketing min/max salaries should be 65000/70000");
    assert!(stdout.contains(",60000,65000"), "Sales min/max salaries should be 60000/65000");
