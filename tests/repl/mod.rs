...
fn test_repl_basic_commands() {
    ...
    let mut process = Command::new("cargo")
        .args(&["run", "--bin", "sqawk", "tests/data/sample.csv", "--interactive"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
    ...
    stdin.write_all(test_commands.as_bytes()).expect("Failed to write to stdin");
