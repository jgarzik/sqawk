...
fn test_malformed_csv_with_error_recovery() {
    ...
    let rows = table.rows();
    assert_eq!(rows[0][1].to_string(), "Alice", "First row should have Alice");
    ...
    for row in table.rows() {
        if row.len() > 1 {
            ...
            if name == "Alice" { has_alice = true; }
            else if name == "Bob with an unclosed quote\"" { has_bob = true; }
            else if name == "Charlie" { has_charlie = true; }
        }
    }
