...
fn test_distinct_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.contains("Alice,Engineering"),
        "Missing Alice,Engineering"
    );
    assert!(stdout.contains("Bob,Marketing"), "Missing Bob,Marketing");
    assert!(
        stdout.contains("Charlie,Engineering"),
        "Missing Charlie,Engineering"
    );
...
fn test_distinct_with_where() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stderr.contains("WHERE comparison"),
        "Missing 'WHERE comparison' in stderr"
    );
    ...
    let expected_entries = vec!["Alice,Developer", "Charlie,Manager"];
    ...
    assert!(
        ...
        "Found unexpected role: Specialist"
    );
    assert!(
        !stdout.contains("Analyst"),
        "Found unexpected role: Analyst"
    );
    assert!(
        !stdout.contains("Representative"),
        "Found unexpected role: Representative"
    );
...
fn test_distinct_with_order_by() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.contains("name,department"),
...
fn test_distinct_with_join() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stderr.contains("INNER JOIN"),
        "Missing JOIN information in stderr"
    );

    // Check output - should include unique combinations from the join
    assert!(
        stdout.contains("users.name,orders.product_id"),
        "Missing header"
    );

    // Check for expected entries after join
    let expected_entries = vec!["John,101", "John,103", "John,104", "Jane,102", "Jane,105"];

    // Make sure all expected entries are present
    for entry in &expected_entries {
        assert!(
            stdout.contains(entry),
            "Missing expected join result: {}",
            entry
        );
    }
...
fn test_distinct_with_select_star() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ...
        "Found unexpected department: Marketing"
    );
    assert!(
        !stdout.contains("Finance"),
        "Found unexpected department: Finance"
    );
    assert!(!stdout.contains("HR"), "Found unexpected department: HR");
    assert!(
        !stdout.contains("Sales"),
        "Found unexpected department: Sales"
    );
...
fn test_distinct_without_id() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        alice_developer_count, 1,
        "Expected exactly 1 Alice,Engineering,Developer entry, found {}",
        alice_developer_count
    );
